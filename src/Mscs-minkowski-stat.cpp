/*!
 * \file Mscs-minkowski-stat.cpp
 *
 *  Project: cpems
 *  Created on: Jun 7, 2018 7:14:12 PM
 *  Author: blew
 */
////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////
...
minkowski_fs_MFstat::minkowski_fs_MFstat(long node_no, string sim_type, long sim_stnum, long sim_ennum, long sm_num, double* sm, long mm_num, string* mmf, long th_num, string mask_name) {
	...
	sim_type_num=0;
	if (sim_type.find("Q",0,1)!=string::npos)  { tstr[sim_type_num]="Q";  sim_type_num++;  }
	if (sim_type.find("V",0,1)!=string::npos)  { tstr[sim_type_num]="V";  sim_type_num++;  }
	if (sim_type.find("W",0,1)!=string::npos)  { tstr[sim_type_num]="W";  sim_type_num++;  }
	if (sim_type.find("VW",0,2)!=string::npos) { tstr[sim_type_num]="VW"; sim_type_num++;  }
	...
	for (m=0;m<mm_num;m++) { 
		if (mmf[m].find("HP_2",0,4)!=string::npos) qlong.addq(48); else
			if (mmf[m].find("HP_4",0,4)!=string::npos) qlong.addq(192); else
				if (mmf[m].find("HP_8",0,4)!=string::npos) qlong.addq(768); 
...
/*
 * Comment: Routine to be reimplemented if needed
 * 
 * author: blew
 * date: Jun 7, 2018 3:12:49 PM
 *
 */
