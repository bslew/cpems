...
//		static const int subregionsCount1d=2;
		typedef struct {
			...
			unsigned long maxDepth; //!< maximal depth below which the tree does not continue. if -1 then not used (used alternatively to minObjectsCount)
			unsigned long curDepth; //!< current depth. 0 for root subdomain
			subDomain **sub; //!< points to array of sub-domains
			int subDomainsCount; //!< number of sub-domains in this domain (only 1 level down and only if the subdivision is done- this basically results from the partitioning scheme)
			...
			bool leaf; //! true if the domain is the leaf domain; false otherwise
	...
	public:
		...
		subDomain(const mscsVector<cpedsPoint3D>* points, const subDomain_region_t *domain, long minPoints);
		//! sub-domain constructor (do not use it for the initialization of this class. It is used internally recursively)
		subDomain(subDomain* parent, const subDomain_region_t &newSubDomain);
		virtual ~subDomain();
...
//		void tree(const subDomain *domain=NULL);
		...
		*/
		...
		bool partiallyInDomain(const subDomain_region_t& r, const subDomain* D=NULL) const;
		/*!
		...
		*/
		...
		/*!
			...
			@param Nmin - minimal number of particles inside of the spherical region (eg. more than particles then the behavior is not guaranteed .)
			@param Nmax - maximal number of particles inside of the spherical region, if this is set incorrectly (eg. more than particles then the behavior is not guaranteed .)
			@param hsml - pointer to the smoothing length - the radius containing the found particles
			@param subDivisionsCount - accuracy parameter defining maximal number of iterations in trying to set the smoothing length
			...
			p0 from within the range [Nmin,Nmax]. The number of iterations is increased on every search radii subdivision.
			This definitions is relative to the domain size as the initial hsml is set to the diagonal of the box. So the 
			default value subDivisionsCount=13 gives accuracy of 2^-13*hsml_ini which is O(1e-4)*hsml_ini.
			...
			\author Bartosz Lew
		*/
		mscsVector<long> getNeighbors(cpedsPoint3D p0, long Nmin, long Nmax, double* hsml=NULL, long subDivisionsCount=13);
...
//		vector<long>* getContainingDomainPointIdx(const subDomain_region_t& r, subDomain** sd=NULL, bool ignoreRegionSize=false);
		vector<long>* getContainingDomainPointIdx(const subDomain_region_t& r, subDomain** sd=NULL, bool newSearch=false);
		vector< vector<long>* >* getOverlappingDomainPointIdx(const subDomain_region_t& r, subDomain** sd=NULL, bool newSearch=false, bool claenUp=false);
		...
		/*!
			...
			\author Bartosz Lew
		*/
		mscsVector<long> getPointsIdx(const cpedsPoint3D& p0, double r, subDomain* D=NULL, bool ignoreRegionSize=false, bool cubicalRegion=false);
		
		mscsVector<long> getShellPointsIdx(const cpedsPoint3D& p0, double r, double dr,subDomain* D=NULL);
		/*!
			\brief see info on void branchParticlesCount(vector< vector<double> >& pcount, bool leafsOnly=false);
			\details 
			
			\date Apr 5, 2013, 10:20:30 AM
			\author Bartosz Lew
		*/
		vector< vector<double> > getBranchParticlesCount(bool leafsOnly=false);
		
		void setMaxDepth(int md) { _def.maxDepth=md; }
	...
	protected: 
		...
		*/
		...
		/*!
			\brief returns a vector of sub-domains information
			\details 
			@param pcount - a vector that contains domains information
			each row in this vector contains the following information:\n
			_def.region.xmin, _def.region.xmax
			_def.region.ymin, _def.region.ymax
			_def.region.zmin, _def.region.zmax
			_def.idx->size() - number of particles in this subdomain
			0 if not leaf and 1 if is leaf
			
			IMPORTANT: if you change the amount of stuff that this vector carries (now 8 entries at the beginning, followed by a list of indexes of particles located in this subdomain)
			then REMEMBER to also update the code in 
			mscsFunction3dregc& mscsFunction3dregc::mkDensityFieldCube(subDomain_region_t r, subDomain_region_t treeScheme, const mscsVector<cpedsPoint3D>& positions, const mscsVector<double>* weights, int part, long maxDepth, bool calcMass)
			method or else the density calculations will have wrong values.
			
						
			\date Aug 7, 2012, 4:12:56 PM
			\author Bartosz Lew
		*/
		void branchParticlesCount(vector< vector<double> >& pcount, bool leafsOnly=false);
