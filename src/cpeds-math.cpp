...
long cpeds_get_cols_num_first_ln(strarg fn,char * lastc) {
	...
	f=fopen(fn,"r"); if (f==NULL) { printf("error: no such file (%s)\n",fn); return 0; }
...
cpeds_queue<long>* cpeds_get_txt_file_cols_rows(strarg fn, long scanRowsMax) {
	long cols=0,lines=0;
	char c;
	cpeds_queue <long> *q;
	FILE * f;
...
double cpeds_bilinear_interpolation(double x1, double x2, double y1, double y2, double f11, double f12, double f21, double f22, double x, double y) {
	double fx1=cpeds_extrapolate_linear(x,x1,f11,x2,f21);
	double fx2=cpeds_extrapolate_linear(x,x1,f12,x2,f22);
	return cpeds_extrapolate_linear(y,y1,fx1,y2,fx2);
}
/* ******************************************************************************************** */
