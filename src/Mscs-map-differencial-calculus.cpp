...
map_class * map_class::partial_derivative_phi(string _object_name, map_class * u) {
  double dphi;
  ...
  orig_ordering = (*u).map_ordering;

  // clone u into a new object uphi
  uphi = new map_class(_object_name, *u); // clone data onto new maps

  // load the proper coordinates into the uphi object
  if ( (*uphi).coord_loaded == 0) { 
    if ( (*uphi).map_ordering == 0) { (*uphi).conv_ring2nest((*uphi).map); }
    (*uphi).set_map_coord(0,0); 
  }

  // convert to ring ordering for calculation
  if ( (*uphi).map_ordering == 1) { (*uphi).conv_nest2ring((*uphi).map); }
  if ( (*u).map_ordering == 1) { (*u).conv_nest2ring((*u).map); }
  ...
  for (i=0;i<ring_num;i++) {
    ...
    for (j=0;j<pixinring_num;j++) {
      ...
      if ((*u).map->m[k] == 0) { (*uphi).map->m[l]=0; (*uphi).map->m[m]=0; } // mark empty pixels to extend the mask  latter
      if ((*u).map->m[l] == 0) { (*uphi).map->m[k]=0; (*uphi).map->m[m]=0; } // mark empty pixels to extend the mask  latter
      if ((*u).map->m[m] == 0) { (*uphi).map->m[k]=0; (*uphi).map->m[l]=0; } // mark empty pixels to extend the mask  latter

      /*       (*uphi).map->T[k] = 0.5 * ( (*u).map->T[m] + (*u).map->T[l] - 2.0*((*u).map->T[k]) ) / dphi;  // WHY THIS IS NOT THE SAME AS THE LINE BELOW ???? */ 
               (*uphi).map->T[k] = 0.5 * ( ((*u).map->T[k] - (*u).map->T[m]) / dphi + ( (*u).map->T[l] - (*u).map->T[k] ) / dphi );
/*       (*uphi).map->T[k] = 0.5*((*u).map->T[k] - (*u).map->T[m])/dphi; */
  ...
  if (orig_ordering == 1) { (*uphi).conv_ring2nest((*uphi).map); (*u).conv_ring2nest((*u).map); }
...
// the differentiation is performed directly on the map; not in the SH domain
map_class * map_class::cov_derivative_phi(string _object_name, map_class * u) {
  double th;
  map_class *uphi;
  long i;

  printf(" * calculating covariant derivative of the map with respect to phi\n");
  uphi = partial_derivative_phi(_object_name, u);
...
map_class * map_class::partial_derivative_th(string _object_name, map_class * u) {
...
/*   long ns3=3*nside; // DEPRECIATED */
  ...
  // clone u into a new object uth
  uth = new map_class(_object_name,*u); // the ordering will be  after u

  // load the proper coordinates into the uth object
  if ( (*uth).coord_loaded == 0) { 
    if ( (*uth).map_ordering == 0) { (*uth).conv_ring2nest((*uth).map); }
    (*uth).set_map_coord(0,0); 
  }

  // convert to ring
  if ( (*uth).map_ordering == 1) { (*uth).conv_nest2ring((*uth).map); } 
  if ( (*u).map_ordering == 1) { (*u).conv_nest2ring((*u).map); } 


  // initiate 2D array and interpolation arrays
  ring_num = cpeds_get_ring_num_healpix(nside);
  Dth = new matrix <double>(ring_num,ns4);
  ...
  //********************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************
  //********************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************
  // CODE BELOW IS DEPRECIATED -- TO BE REMOVED SOON
  //********************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************
  //********************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************
  ...
  (*uth).load_conv_tabs((*uth).nside,"r2n");
  ...
  ring = 0;
  for (i=0;i<pix_num;i++) {
    th = PIsnd - (*uth).map->n[i].b;    phi = (*uth).map->n[i].l; // th and phi coords of the i'th pixel in nested ordering
    ...
    l=(*uth).r2n_conv_tab[l];  // convert it to the ring pixel number //   THIS IS TEMPORARY STUFF  !!!
    m=(*uth).r2n_conv_tab[m];  // convert it to the ring pixel number //   THIS IS TEMPORARY STUFF  !!!

    //calculate the derivative -i.e. the differencial ratio
    (*uth).map->T[i] = 0.5 * ( ((*u).map->T[l] - (*u).map->T[i])/dthij + ((*u).map->T[m] - (*u).map->T[i])/dthik );

    if ((*u).map->m[i] == 0) { (*uth).map->m[l]=(*uth).map->m[m]=0; } // mark empty pixels to extend the mask  latter
    if ((*u).map->m[l] == 0) { (*uth).map->m[i]=(*uth).map->m[m]=0; } // mark empty pixels to extend the mask  latter
    if ((*u).map->m[m] == 0) { (*uth).map->m[i]=(*uth).map->m[l]=0; } // mark empty pixels to extend the mask  latter

  }

  //********************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************
  //********************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************
  // CODE ABOVE IS DEPRECIATED -- TO BE REMOVED SOON
  //********************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************
  //********************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************
  ...
  if (orig_ordering == 1) { (*uth).conv_ring2nest((*uth).map); (*u).conv_ring2nest((*u).map);}
...
// the differentiation is performed directly on the map; not in the SH domain
map_class * map_class::cov_derivative_th(string _object_name, map_class * u) {
  map_class *uth;
  printf("  * calculating covariant derivative of the map with respect to theta\n");
  uth = partial_derivative_th(_object_name,u);
