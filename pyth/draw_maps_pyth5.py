...
figure(figsize=(5,2.5), dpi=dpires)
...
#m1 = 
m1=Basemap(projection='ortho',lon_0=0,lat_0=0) #.5*(lons[0]+lons[-1])
...
Cld=asciidata.open(Cldata)
ld=Cld[0].tonumarray()
Cd=Cld[1].tonumarray()
Cl=asciidata.open(Clsim)
l=Cl[0].tonumarray()
C=Cl[1].tonumarray()

C=C*l*(l+1)/2/3.1415926*1e12
Cd=Cd*ld*(ld+1)/2/3.1415926*1e12

# make plot
plot_fontsize=5
plot(ld,Cd,'ro-', linewidth=1, markersize=3, label='data')
plot(l,C,'k-', linewidth=2, markersize=0, label='sim')
xlabel('l', fontsize=plot_fontsize)
ylabel('$l(l+1)/(2\pi) C_l [\mu K]^2$', fontsize=plot_fontsize)
#$\frac{l(l+1)}{2\pi}C_l [\mu K]^2$
#setp(xticklabels, fontsize=plot_fontsize)
#setp(yticklabels, fontsize=plot_fontsize)
xticklabels = getp(gca(), 'xticklabels')
yticklabels = getp(gca(), 'yticklabels')
setp(xticklabels, 'color', 'k', fontsize=plot_fontsize)
setp(yticklabels, 'color', 'k', fontsize=plot_fontsize)
