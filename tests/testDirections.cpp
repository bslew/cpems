...
string _programVersionString, _track, _planet, _datetime;
bool _calcMoon, _calcPlanet, _utime2JD, _novaVSnovas, _novaReversabilityTest,
    _testRTlstCalculation;
bool _calcSolstice;
bool _sunRiseAndSet;
...
void testConversionDeg2HMSDMS(double ra, double dec, double jdutc,
                              cpedsMsgs& msgs);
...
int main(int argc, char** argv) {
    ...
    if (_novaVSnovas)
        novaVSnovas(msgs);
    if (_novaReversabilityTest)
        novaReversabilityTest(msgs);
    if (_testRTlstCalculation)
        testRTlstCalculation();
...
void parseOptions(int argc, char** argv) {
    ...
    try {
        ...
        CmdLine cmd(
            ...
            "This program calculates the coordinates of bodies in the Solar "
            "System, and controls the RT32 telescope to track them in the "
            "sky.\n\n. "
            "\n"
            "example usage: \n"
        ...
        //     program simulates the RT4 scan motion, sky rotation during \n\n.
        //     ', 0.1 );
        ...
        ValueArg<double> lon(
            ...
            false, 18.56406, "double");
        cmd.add(lon);
        ValueArg<double> lat(
            "", "lat",
            "geodetic latitude of the observatory location (-90,90) [deg]",
            false, 53.09546, "double");
        cmd.add(lat);
        ValueArg<double> altitude(
            "", "alt",
            "altitude of the observatory over the ellipsoid (133) [m]", false,
            133.61, "double");
        cmd.add(altitude);
        ValueArg<double> pressure("P", "press",
                                  "current atmospheric pressure (1012) [mbar]",
                                  false, 1012.0, "double");
        cmd.add(pressure);
        ValueArg<double> temp("T", "temp",
                              "current atmospheric temperature (20) [Celsius]",
                              false, 20.0, "double");
        cmd.add(temp);
        ValueArg<double> JD("", "JD",
                            "JD for which to calculate the stuff (default: 0 = "
                            "current UT time) [JD]",
                            false, 0, "double");
        cmd.add(JD);
        ...
        ValueArg<double> da("", "da",
                            ...
                            "(default: 0 = current JD setting) [JD]",
                            false, 0, "double");
        cmd.add(da);
        ValueArg<double> dt(
            "", "dt",
            "time step in calculating moon positions (default: 24 h) [h]",
            false, 24, "double");
        cmd.add(dt);
        ValueArg<double> timeLimit("", "period",
                                   "time period over which to calculate the "
                                   "ephemerides (default: 30 JD) [JD]",
                                   false, 30, "double");
        cmd.add(timeLimit);
        ValueArg<double> ut("", "ut",
                            "unix UTC time to be converted to JD UTC date "
                            "(default:  1.35577627082611608505E+09) ",
                            false, 1.35577627082611608505E+09, "double");
        cmd.add(ut);
        //
        SwitchArg solstice("", "calcSolstice",
                           "calculate next solstice (default: false)", false);
        cmd.add(solstice);
        SwitchArg sunRiseAndSet(
            "", "calcSunRiseAndSet",
        ...
        SwitchArg moon("", "moon",
                       "show moon direction in the sky and for the following "
                       "month (default: false)",
                       false);
        cmd.add(moon);
        //		SwitchArg track("","track", "send RT32 to the selected
        // object (default: false)", false);	cmd.add( moon );
        SwitchArg utime2JD("", "utime2JD",
                           "convert unix time to JD. Use --ut option to set "
                           "the unix time (default: false)",
                           false);
        cmd.add(utime2JD);
        SwitchArg novaVSnovas("", "novaVSnovas",
                              "compare coordinates conversion radec->ah "
                              "between the two libraries (default: false)",
                              false);
        cmd.add(novaVSnovas);
        SwitchArg novaReversabilityTest(
            "", "novaRev",
            "test reversability of nova directions conversion "
            "ra,dec->ah->ra,dec (default: false)",
            false);
        cmd.add(novaReversabilityTest);
        SwitchArg testRTlstCalculation("", "testRTlst",
                                       "test continuity of lst calculation in "
                                       "RT control system (default: false)",
                                       false);
        cmd.add(testRTlstCalculation);

        ValueArg<double> ra("", "ra",
                            "right ascension for testing conversion from deg "
                            "to hms [deg] [0,360) (default: -1 = not used ) ",
                            false, -1.0, "double");
        cmd.add(ra);
        ValueArg<double> dec("", "dec",
                             "declination for testing conversion from deg to "
                             "dms [deg] [-90,90] (default: -100 = not used ) ",
                             false, -100.0, "double");
        cmd.add(dec);
        //

        //		ValueArg<string> planet("","planet","send RT32 to the
        // selected object ",false,"","string"); cmd.add(outfile);
        //		ValueArg<string> outdir("","odir","output directory
        // name",false,"./","string"); cmd.add(outdir);
        //
        ...
        //		ValueArg<string> track("", "traj", "track","send RT32 to
        // the selected object", false,"none", allowedStrNew);	cmd.add( track
        //);
        ValueArg<string> planet("", "planet", "planet name (default: none)",
                                false, "none", allowedStrNew);
        cmd.add(planet);
        allowedStr.clear();

        //     UnlabeledValueArg<string> input_file("input_file","file to
        //     plot","nofile","string");	cmd.add( input_file );
        //     ValueArg<string> mask("m","mask","wheather to mask the file
        //     before doing statistics (prefix)",false,"","string");
        //     cmd.add(mask); SwitchArg mask_from_here("","MM", "take mask from
        //     current directory rather than from the default directory",
        //     false);	cmd.add( mask_from_here ); SwitchArg
        //     dont_save_mask("","dont_save_mask", "do not print to file masked
        //     pixels; useful for -Tn-txt savings", false);	cmd.add(
        //     dont_save_mask ); ValueArg<string> proj("p","proj","projection to
        //     use [mall - Mollweide/ aitoff etc] (default:
        //     mall)",false,"mall","string"); cmd.add(proj); ValueArg<string>
        //     ft("f","ft","input file type [bin, txt] (default:
        //     bin)",false,"bin","string"); cmd.add(ft); ValueArg<string>
        //     ord("","ord","ordering of the map [ring, nest] default:
        //     nest",false,"nest","string"); cmd.add(ord); SwitchArg
        //     mink("M","Mink", "plot also the minkowski functionals for the
        //     map", false);	cmd.add( mink ); ValueArg<long> mlevels("",
        //     "mlev", "number of levels to calculate the minkowski functionals
        //     (default: 100)", false,100,"long");	cmd.add( mlevels );
        ...
        _sunRiseAndSet = sunRiseAndSet.getValue();
        _ut = ut.getValue();
        ...
        //		_outdir = outdir.getValue(); 	if (_outdir=="")
        //_outdir=".";
...
void calculateMoon(cpedsMsgs& msgs) {
    ...
    moonNowAh.check().print_direction(
        "Ah (topocentric, no refraction, UT1=UTC, no polar motion)");
    moonNowAh = moonNow.toAh(obs, JDutc, 0, 0, 0, _P, _T, true);
    moonNowAh.check().print_direction(
        "Ah (topocentric, with refraction, UT1=UTC, no polar motion)");
    msgs.say("distance: %lf [km]", moon.distance(_JD) / 1000, High);

    msgs.say("position angle of the bright limb: %lf",
             moon.brightLimbPA(JDutc) * PI180inv, High);
    msgs.say("illuminated disk fraction: %lf",
             moon.illuminatedDiskFraction(JDutc), High);
    msgs.say("phase angle: %lf", moon.opticalPhase(JDutc) * PI180inv, High);

    DirectionRaDec dsc = moon.getDarkSideCenter(JDutc, &dscSize);
    dsc.print_direction("dark side disk center radec (topocentric, no "
                        "refraction, UT1=UTC, no polar motion):");
    moonNow.toJD(CPEDS_JD2000);
    moonNow.check().print_direction(
        "radec (topocentric, no refraction, UT1=UTC, no polar motion):");
    moonNow.toJD(JDutc);
    moonNow.check().print_direction(
        "radec (topocentric, no refraction, UT1=UTC, no polar motion):");
    moonNowAh = dsc.toAh(obs, JDutc, 0, 0, 0, _P, _T, true);
    moonNowAh.check().print_direction(
        "dark side disk center Ah (topocentric, with refraction, UT1=UTC, no "
        "polar motion)");
    ...
    while (JD <= timeLimit) {
        i++;
        JD += dt;
    } // first pass to learn how big matrix to allocate
    N = i;
    ...
    while (JD <= timeLimit) {
        ...
        i++;
        JD += dt;
        //			msgs.say("calculating position, step: %li",
        // i,Medium);
    }
    ...
    fprintf(f, "5 - bright limb mid point position angle [deg] (north cusp, "
               "eastwards)\n");
...
void calculatePlanet(string planetName, cpedsMsgs& msgs) {
    ...
    std::cout << "JDutc: " << std::setprecision(15) << JDutc << ", " << yyyy
              << "-";
    std::cout.width(2);
    std::cout.fill('0');
    ...
    nowAtAh.check().print_direction(
        "Ah (topocentric, no refraction, UT1=UTC, no polar motion)");
    nowAtAh = nowAt.toAh(obs, JDutc, 0, 0, 0, _P, _T, true);
    nowAtAh.check().print_direction(
        "Ah (topocentric, with optical refraction, UT1=UTC, no polar motion)");
    ...
    fprintf(f, "4 - planet center A [deg] (for JD, topocentric, with optical "
               "refrection)\n");
    fprintf(f, "5 - planet center h [deg] (for JD, topocentric, with optical "
               "refrection)\n");
    fclose(f);
    //	fn.save("moon-radec.txt");
}
...
void calculateNextSolstice(cpedsMsgs& msgs) {
    ...
    while (JD <= timeLimit) {
        ...
        string tstr =
            cpeds_JDToYMDhms(JD, "%li-%02li-%02li %02li:%02li:%06.3lf");
        fprintf(f, "%s %.15lf %.10lf %.10lf\n", tstr.c_str(), JD, nowAt.lon(),
                nowAt.lat());
...
void calculateSunRiseAndSet(cpedsMsgs& msgs) {
    ...
    double JD = int(JDutc);
    DirectionRaDec nowAt;
    enum ElevTarget { UPPER_LIMB, LOWER_LIMB, CENTER };
    auto getElevationVsJD = [&](double JDst, double JDen, double dt,
                                ElevTarget target = CENTER) {
        JD = JDst;
        mscsFunction elevVsJD;
        while (JD <= JDen) {
            nowAt = planet.nowAtRaDec(JD, JD, 0, false);
            ...
            if (target == UPPER_LIMB) {
                ...
                if (inSpaceSunUpperLimbElevation < -calcLimit ||
                    inSpaceSunUpperLimbElevation > calcLimit) {
                    JD += dt;
                    continue;
                }
    ...
    auto stringifyLimbType = [](ElevTarget target) {
        switch (target) {
        case UPPER_LIMB:
            return string("upper limb");
        case LOWER_LIMB:
            return string("lower limb");
        case CENTER:
            return string("center");
        default:
            return string("unknown limb type");
        }
    };

    auto printSunRiseSetTimes = [&](const vector<double>& nullElevations,
                                    ElevTarget target) {
        for (auto nullElev : nullElevations) {
            string tstr = cpeds_JDToYMDhms(
                nullElev, "%li-%02li-%02li %02li:%02li:%06.3lf");
            printf("Sun %s rise/set on: %s\n",
                   stringifyLimbType(target).c_str(), tstr.c_str());
            printf("JD: %.15lf\n", JD);
            printf("dt: %.15lf [s]\n", _dt * 3600);
        }
    };

    // FILE* f = fopen("sunRiseAndSet.txt", "w");
    mscsFunction ctrVsJD = getElevationVsJD(JD - 0.5, JD + 0.5, _dt, CENTER);
    auto nullElevations = ctrVsJD.findRoot();
    ctrVsJD.save("sunRiseAndSet.ctr.txt");
    printSunRiseSetTimes(nullElevations, CENTER);

    JD = int(JDutc);
    mscsFunction upperLimbVsJD =
        getElevationVsJD(JD - 0.5, JD + 0.5, _dt, UPPER_LIMB);
    nullElevations = upperLimbVsJD.findRoot();
    upperLimbVsJD.save("sunRiseAndSet.upperLimb.txt");
    printSunRiseSetTimes(nullElevations, UPPER_LIMB);

    JD = int(JDutc);
    mscsFunction lowerLimbVsJD =
        getElevationVsJD(JD - 0.5, JD + 0.5, _dt, LOWER_LIMB);
    nullElevations = lowerLimbVsJD.findRoot();
    lowerLimbVsJD.save("sunRiseAndSet.lowerLimb.txt");
    printSunRiseSetTimes(nullElevations, LOWER_LIMB);

    // string tstr = cpeds_JDToYMDhms(JD, "%li-%02li-%02li
    // %02li:%02li:%06.3lf");
    // printf("(min dec) Solstice on: %s\n", tstr.c_str());
    // printf("JD: %.15lf\n", JD);
    // printf("dt: %.15lf [s]\n", _dt * 3600);
...
void novaVSnovas(cpedsMsgs& msgs) {
    ...
    for (long yy = 1980; yy <= 2013; yy++) {
        for (long i = 1; i <= 12; i++) {
            ...
            //			printf("JD RT : %.10lf  (month count from
            ...
            printf("x: %lf, JD: %.10lf, diff(app-RTctrl): %lf, diff(app-mean): "
                   "%.10lf\n",
                   x, JD, d, (alst - mlst));
        }
    }
...
double RTcontrol_local_sidereal_time(double UT, double JD, double longitude) {
    ...
    d = fmod(1.55811454642E0 + fmod(d + d, 2.0E0) +
                 d * (0.547581870159E-2 + d * (1.61549E-15 - d * 1.473E-24)),
             2.0E0);
...
double RTcontrol_JulianDay(int tye, int tmo, int tda) {
    ...
    return (double)(1471086l + 365 * j + (j + 1000004l) / 4 + 30 * i + i / 2 +
                    ((i / 7) * i) % 2 + i / 12 + tda + (j + 1000400l) / 400 -
                    (j + 1000100l) / 100 + 7502);
};
void testRTlstCalculation() {
    ...
    for (long Y = 2000; Y <= 2013; Y++) {
        ...
        for (long M = 1; M <= 12; M++) {
            ...
            for (long h = 0; h < 24; h++) {
                ...
                sidereal_time(JD, 0, 0, 1, 0, 0, &alst);
                alst = alst * 3600 + lon;
                if (alst > 86400)
                    alst -= 86400;
                rtlst = RTcontrol_local_sidereal_time(UT, JDay, lon);
                printf("yy:%lf mm: %lf dd: %lf, h: %li m: %lf s: %lf, JD: "
                       "%.10lf, UT: %lf, "
                       "lst: %.10lf alstNOVAS: %.10lf, diff: %lf"
                       "\n",
                       yy, mm, dd, h, m, s, JDay, UT, rtlst, alst,
                       alst - rtlst);
                printf("  JDcpeds: %.10lf\n", JD);
                printf("--------------------\n");
            }
